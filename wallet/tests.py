...
class ToUpWalletServiceTests(TestCase):
    ...
    def test_top_up_increases_balance(self) -> None:
        self.walletservice.top_up_wallet(account=self.account, amount=500000)
    ...
    def test_top_up_rejects_zero_or_negative_amount(self) -> None:
        with self.assertRaises(ValueError):
            self.walletservice.top_up_wallet(account=self.account, amount=0)
        with self.assertRaises(ValueError):
            self.walletservice.top_up_wallet(account=self.account, amount=-10000)
...
class DebitWalletServiceTests(TestCase):
    ...
    def test_debit_decreases_balance(self) -> None:
        self.walletservice.debit_wallet(account=self.account, amount=40000)
        self.wallet.refresh_from_db()
        self.assertEqual(self.wallet.balance, 60000)

    def test_debit_creates_payment_transaction(self) -> None:
        self.walletservice.debit_wallet(account=self.account, amount=50000)
        record = self.wallet.transactions.get()  # pyright: ignore[reportAttributeAccessIssue]
        self.assertEqual(record.type, WalletTransaction.TransactionType.PAYMENT)

    def test_debit_rejects_when_balance_insufficient(self) -> None:
        with self.assertRaises(InsufficientBalanceError):
            self.walletservice.debit_wallet(account=self.account, amount=99999999)
        self.wallet.refresh_from_db()
